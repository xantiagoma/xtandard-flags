 ...
 *
 *   bun run examples:flags-sdk      # or: elysia | hono | express | …
 *
...
const EXAMPLES: Record<string, ExampleConfig> = {
  "flags-sdk": {
    dir: "examples/flags-sdk",
    kind: "server",
    seed: ["bun", "run", "seed"],
    run: ["bun", "run", "dev"],
    preferredPort: 3000,
    note: "Next.js app (Vercel Flags SDK via OpenFeature)",
  },
  elysia: {
    dir: "examples/elysia",
    kind: "server",
  ...
  "openfeature-redis": {
    dir: "examples/openfeature-redis",
    kind: "script",
    run: ["bun", "run", "start"],
...
if (cfg.note) console.log(`ℹ︎  ${name}: ${cfg.note}`);

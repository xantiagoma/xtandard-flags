...
import { inlineSegments, resolveSegments, usesEmbeddedSegments } from "./segments.ts";
...
export function compileDraft(draft: Draft, options: CompileOptions): Snapshot {
  ...
  // Single-key `inSegment` is inlined above. `notInSegment` and array (OR)
  // `inSegment` can't be, so embed the resolved segments for the evaluator to
  // check membership — only when actually used.
  const embed = options.segments && usesEmbeddedSegments(flags);
  return {

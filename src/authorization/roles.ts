 ...
 * {@link FlagsAction}s it grants, then allows an action when *any* of the
 * principal's roles grants it.
 *
 * The policy is a flat `Record<roleName, FlagsAction[] | "*">` — `"*"` is a
...
export const ALL_ACTIONS: readonly FlagsAction[] = [
  "project:read",
  "project:create",
  "project:update",
  "project:delete",
...
export function rolesAuthorization(
  ...
  const grants = new Map<string, Set<FlagsAction> | null>();

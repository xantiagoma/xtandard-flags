...
export interface PostgresStorageOptions {
  ...
  url?: string;
  /** Table name (default `"xtandard_flags"`). Must be a safe SQL identifier. */
  table?: string;
}

/**
 * A {@link FlagsStorage} backed by Postgres, plus a `close()` method that ends
 * the underlying pool — but only the one this adapter created. A client you
 * passed in is left for you to manage.
 */
export interface PostgresFlagsStorage extends FlagsStorage {
...
/**
 * Create a Postgres-backed {@link PostgresFlagsStorage}. The table is created
...
export function createPostgresStorage(options: PostgresStorageOptions): PostgresFlagsStorage {
  ...
  } satisfies PostgresFlagsStorage;
}

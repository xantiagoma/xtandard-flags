 ...
 * The flag key is stored verbatim as the document `_id`, and the value is stored
 * directly as a BSON field — MongoDB persists arbitrary objects, arrays, and
 * primitives, so no JSON (de)serialisation round-trip is needed.
 *
 ...
 * ## Why no `watch`
 *
 * MongoDB change streams (the natural way to back {@link WatchableFlagsStorage})
...
export interface MongoStorageOptions {
   ...
   */
  url?: string;
  /** Database name. Defaults to `"xtandard_flags"`. */
  dbName?: string;
  /** Collection name. Defaults to `"flags_kv"`. */
  collectionName?: string;
}

/**
 * A {@link FlagsStorage} backed by MongoDB, plus a `close()` method that
 ...
 * {@link FlagsStorage}, not a {@link import("./contract.ts").WatchableFlagsStorage}.
 */
export interface MongoFlagsStorage extends FlagsStorage {
...
interface KvDoc extends Document {
  /** The flag key, stored verbatim as the Mongo document id. */
  _id: string;
...
/**
 * Create a MongoDB-backed {@link MongoFlagsStorage}. Connection is lazy: the
  ...
  async function getCollection(): Promise<Collection<KvDoc>> {
    connecting ??= (async () => {
      if (!client) {
        ...
        if (url === undefined) {
          throw new Error(
            '@xtandard/flags/storage/mongodb requires either a "client" or a "url" option.',
          );
        }

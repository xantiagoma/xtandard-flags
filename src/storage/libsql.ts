 ...
 * // const storage = createLibsqlStorage({ url: "file:flags.db" });
 * ```
 *
 * @module
 */
...
export interface LibsqlStorageOptions {
  ...
  /** Table name (default `"xtandard_flags"`). Validated as a safe identifier. */
  table?: string;
}

/** A {@link FlagsStorage} backed by libSQL/Turso, plus `close()`. */
export interface LibsqlFlagsStorage extends FlagsStorage {
...
/**
 * Create a libSQL/Turso-backed {@link LibsqlFlagsStorage}. The table is created on
 * first use; connection (when constructed from `url`) is lazy and shared.
 *
 * @example
 * ```ts
 * import { createLibsqlStorage } from "@xtandard/flags/storage/libsql";
 ...
 * ```
 */
export function createLibsqlStorage(options: LibsqlStorageOptions): LibsqlFlagsStorage {

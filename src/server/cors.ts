/**
 * Framework-independent CORS for the mounted panel. When `flagsPanel` /
 ...
 * so a **cross-origin embed** (dashboard on `app.example.com`, panel on
...
export interface FlagsCorsOptions {
  /**
 ...
 */
function resolveOrigin(cors: FlagsCorsOptions, requestOrigin: string | null): string | null {
  const { origin } = cors;
  if (origin === "*") {
...
export function preflightResponse(request: Request, cors: FlagsCorsOptions): Response {

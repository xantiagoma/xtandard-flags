 ...
 * resolve under any mount path, and a bootstrap `window.__FLAGS_CONFIG__` blob.
 * Falls back to a minimal built-in page when the UI bundle is absent (e.g.
 * before `bun run build:ui`, or in headless tests).
 *
 * @module
 */
...
function injectInto(html: string, config: BootstrapConfig): string {
  const tags =
    ...
    `<script>window.__FLAGS_CONFIG__=${escapeJson(config)}</script>`;
...
function fallbackHtml(config: BootstrapConfig): string {
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><base href="${baseHref(
    config.basePath,
  )}"><title>${config.title}</title><script>window.__FLAGS_CONFIG__=${escapeJson(
    config,

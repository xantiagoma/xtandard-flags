...
describe("cloudflare-kv — prefix namespacing", () => {
  test("stores under the configured prefix and strips it from getKeys", async () => {
    const kv = fakeKv();
    const storage = createCloudflareKvStorage({ namespace: kv, prefix: "runtime" });
    await storage.setItem("flags/p/e/active", "v1");

    // The underlying binding sees the namespaced key…
    expect(await kv.get("runtime:flags/p/e/active")).toBe('"v1"');
    // …but callers see the bare key.
    expect(await storage.getKeys("flags/")).toEqual(["flags/p/e/active"]);
    expect(await storage.getItem("flags/p/e/active")).toBe("v1");
  });

  test("getKeys pages through the cursor across many keys", async () => {
    const storage = createCloudflareKvStorage({ namespace: fakeKv() });
    for (let i = 0; i < 7; i++) await storage.setItem(`flags/p/e/k${i}`, i);
